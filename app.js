...
var {genGif}= require('./middleware/genGif');
var {getGif}= require('./middleware/giphy');
...
op.on('connection', (socket) => {
    ...
    socket.on('createGifMessage',(msg)=>{
        getGif(msg.query).then((response)=>{
            let url=response;
            op.to(msg.groupName).emit('newGif',genGif(msg.from, url))
        })
        
    })
    socket.on('createLocation', (loc) => {
        op.to(loc.groupName).emit('newLocation', genLocation(loc.user, loc.latitude, loc.longitude))
    })
